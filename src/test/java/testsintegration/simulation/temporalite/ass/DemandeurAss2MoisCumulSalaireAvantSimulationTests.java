...
class DemandeurAss2MoisCumulSalaireAvantSimulationTests extends Commun {
	...
	demandeurEmploi.getRessourcesFinancieresAvantSimulation().setPeriodeTravailleeAvantSimulation(utileTests.creerPeriodeTravailleeAvantSimulation(850, 1101, 2));
	...
	assertThat(simulationMois6).satisfies(simulationMensuelle -> {
		...
		assertThat(ppa.getMontant()).isEqualTo(390);
	    });
	    assertThat(simulationMensuelle.getAides().get(AideEnum.ALLOCATION_SOUTIEN_FAMILIAL.getCode())).satisfies(asf -> {
		assertThat(asf.getMontant()).isEqualTo(117);
	    });
