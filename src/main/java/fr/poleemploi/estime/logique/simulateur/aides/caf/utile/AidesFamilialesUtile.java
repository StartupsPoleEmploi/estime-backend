...
public class AidesFamilialesUtile {
    ...
    private void ajouterAllocationsFamiliales(Map<String, Aide>  aidesPourCeMois, float montant) {
        ...
        Optional<String> detailAideOptional = aideUtile.getDescription(Aides.ALLOCATIONS_FAMILIALES.getNomFichierDetail());
        if(detailAideOptional.isPresent()) {
            allocationsFamiliales.setDetail(detailAideOptional.get());            
        }
    ...
    private void ajouterComplementFamilial(Map<String, Aide> aidesPourCeMois, float montant) {
    	...
    	Optional<String> detailAideOptional = aideUtile.getDescription(Aides.COMPLEMENT_FAMILIAL.getNomFichierDetail());
    	if (detailAideOptional.isPresent()) {
    	    complementFamilial.setDetail(detailAideOptional.get());
    	}
    ...
    private void ajouterAllocationSoutienFamilial(Map<String, Aide> aidesPourCeMois, float montant) {
    	...
    	Optional<String> detailAideOptional = aideUtile.getDescription(Aides.ALLOCATION_SOUTIEN_FAMILIAL.getNomFichierDetail());
    	if (detailAideOptional.isPresent()) {
    	    allocationSoutienFamilial.setDetail(detailAideOptional.get());
    	}
